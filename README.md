...
*   **Analíticas de Evolución Semanal de Postulantes por Origen (Semanas Dinámicas Reales):** Visualización semanal de postulantes ingresados en el padrón global de talentos desglosados dinámicamente por las opciones reales del campo `Origen del Perfil` en sincronía con la vista Lista Detallada de Talent Mixer (`ID: P-TAL-01`) (`Directo ATS`, `LinkedIn InMail`, `Referido`, `Headhunting`, `Portal Empleo`, `Landing Page`, `Manual`, `Sourcing IA`). Las semanas se calculan dinámicamente según la fecha de creación del candidato (`createdAt`, `fecha_creacion` o `updatedAt`), rotuladas por el número de semana ISO y la fecha de inicio del lunes correspondiente (ej. `Sem 30 (20 Jul)`). En este gráfico no se aplican los filtros de búsqueda ni cliente al pertenecer al padrón global fuera de la colección pipeline.
*   **Analíticas de Distribución de Postulantes por Estado Actual:** Gráfico de pipeline (`PipelineChart.tsx`) estructurado en 4 secciones visuales correspondientes a las 4 fases principales (**F1 Descubrimiento**, **F2 Evaluación**, **F3 Cliente** y **F4 Cierre**). Desglosa los 15 estados estandarizados del pipeline con el número de estado y el título exacto de las columnas Kanban (desde `01 - NUEVO EN REVISION` hasta `15 - CANDIDATO SE BAJA (DROP-OUT)`), manteniendo visibles incluso los estados con valor `0`. Reacciona 100% en tiempo real a los filtros generales activos (**Cliente** y **Búsqueda**) sin porcentajes genéricos e incluye un botón `?` explicativo con modal overlay glassmorphism.

*   **Procesos Activos Recientes Agrupados por Responsable Operativo:** Agrupación dinámica de los procesos de selección activos por el campo `Responsable Operativo` (`responsable_operativo` coincidiendo con la pantalla `ID: P-BUS-02`). Cada búsqueda incluye un botón de acción directo `Pipeline F1` que redirige a la pantalla de la Fase 1 Descubrimiento (`ID: P-DIS-01` en `/descubrimiento`) preseleccionando automáticamente la búsqueda seleccionada en el pipeline.
...
*   **Filtrado Estricto de Fase y Aislamiento de Estados (`mapPipelineToEvaluacionCandidates`):** Regla de negocio en `src/lib/evaluacion.ts` que discrimina estrictamente la pertenencia de los registros del pipeline a la Fase 2. Cualquier candidato registrado en la base de datos cuyo `flujo.estado_actual` corresponda a la Fase 1 (`01 - Nuevo (Para Revisión)`, `02 - Contactado`, etc.) o a las Fases 3/4 es ignorado explícitamente (`if (!currentPhase) continue;`), evitando que aparezca erróneamente en la columna de Screening en `ID: P-EVA-01` hasta ser formalmente promocionado.
...
*   **Assessment Técnico Manual y Registro de Trazabilidad (`AssessmentManualCard.tsx`):** Tarjeta interactiva en la ficha del candidato (`P-EVA-02`) y visualización en el tablero general (`P-EVA-01`) para registrar las conclusiones expertas del reclutador sobre documentos o pruebas externas. Soporta redactar fortalezas, debilidades y veredicto con sello de tiempo inmutable inyectado por backend (`fecha_evaluacion` ISO 8601), formateador a la zona horaria del usuario (`Europe/Madrid`), gestión Fail-Fast con Toast de error y consulta rápida con botón `?` en la lista detallada y bloque en la tarjeta Kanban.
...
*   **2026/08/27:** Módulo de Assessment Técnico Manual y Trazabilidad (`ID: P-EVA-02` & `ID: P-EVA-01`):
    *   **Tarjeta UI de Captura y Lectura (`AssessmentManualCard.tsx`):** Componente modular en la ficha del candidato (`ID: P-EVA-02` - `/evaluacion/[id]`) disponible y homologado en todos los estados de la Fase 2 (05 al 08: Screening, Assessment Técnico, En Duda y Descartado). Dispone de un `textarea` multilínea para redactar fortalezas, debilidades y veredicto con contador de caracteres y modo lectura bloqueado con botón "Editar".
    *   **Sello de Tiempo Visual y Localización (Zonas Horarias):** Exposición de la marca de tiempo `"Evaluación registrada el [DD/MM/YYYY] a las [HH:MM]"` convirtiendo la fecha ISO 8601 guardada a la zona horaria del usuario (`Europe/Madrid` / local).
    *   **Server Action & Inmutabilidad Temporal (`actualizarAssessmentManualAction`):** Implementada en `src/actions/pipeline.ts` con consumo de `PATCH /api/v1/pipeline/:id`. El cliente HTTP omite enviar la fecha, la cual es inyectada obligatoriamente por el servidor en la escritura a Firestore.
    *   **Gestión de Errores Fail-Fast:** En caso de fallos de red o errores HTTP, se descarta el guardado optimista local y se notifica al usuario mediante un Toast en rojo solicitando explícitamente el reintento.
    *   **Visualización en Tablero General (`ID: P-EVA-01`):** En la vista Kanban, incorporación del bloque de resumen técnico en la tarjeta `KanbanCard`. En la vista Lista Detallada, adición de la columna *Assessment Técnico* mostrando `resumen_texto` de forma compacta con el botón **`?`** (tooltip popover) para desplegar el resumen completo.
    *   **Suite de Pruebas Automatizadas:** Creación de `tests/assessment_manual.test.js` con 100% de éxito en 5/5 pruebas unitarias (`npm test`), verificación sin errores en compilación TypeScript (`npx tsc --noEmit`) y build de producción Next.js (`npm run build`).
...
*   **2026/08/26:** Homologación de Etiquetas de Estado, Agrupar por Fases y Precisión de Filtros en Gráfico de Postulantes (`ID: P-DSH-01`):
    ...
    *   **Visualización de Estados en Valor 0:** Garantizada la renderización incondicional de los 15 estados en el panel, incluso aquellos con 0 postulantes asociados.
    *   **Corrección de Filtros y Agregación Real:** Eliminación de aproximaciones porcentuales genéricas y corrección del filtrado dinámico por `selectedClient` y `selectedSearch` vinculando adecuadamente identificadores de búsqueda y perfiles.
    *   **Dinamización de Orígenes del Perfil:** Actualización del gráfico `MetricsChart.tsx` ("Evolución Semanal de Postulantes por Origen") para detectar y renderizar dinámicamente las mismas opciones reales presentes en la columna *Origen del Perfil* de la vista Lista Detallada de `P-TAL-01` (`Directo ATS`, `LinkedIn InMail`, `Referido`, `Headhunting`, `Portal Empleo`, `Landing Page`, `Manual`, `Sourcing IA`), asignando paletas de colores coherentes por canal.
    *   **Agrupamiento por Responsable Operativo y Botón Directo Pipeline F1:** Reestructuración de la sección *Procesos Activos Recientes* en `dashboard/page.tsx` agrupando los procesos por el campo `responsable_operativo` (`RESPONSABLE OPERATIVO` de la pantalla `P-BUS-02`), agregando en cada tarjeta un botón de acceso directo `Pipeline F1` que redirige a `ID: P-DIS-01` (`/descubrimiento?search=...`) preseleccionando automáticamente la búsqueda elegida.
    *   **Pruebas Automatizadas:** Actualización de `tests/dashboard_p_dsh_01.test.js` (Tests 7, 10, 11 y 12) validando la presencia de las 4 fases, las 15 etiquetas, los estados en 0, la integración de pipelineItems, las opciones de origen, la agrupación por Responsable Operativo y el botón a Pipeline F1 (**12/12 pruebas pasadas con 100% de éxito**).
...
*   **23/08/2026:** Nueva Vista "Lista Entrevista" en Pantalla ID: `P-EVA-01` (`EntrevistaTable.tsx` y `page.tsx`):
    *   **Nuevo Modo de Visualización "Lista Entrevista":** Integración del 4to botón selector en la barra de vistas de la pantalla `ID: P-EVA-01` (`/evaluacion`), posicionado inmediatamente a continuación del botón *"Lista detallada"*, con la secuencia: `[Kanban] [Lista detallada] [Lista Entrevista] [Lista Test Personalidad]`.
    *   **Columnas del Smart Scorecard — Entrevista de Screening (`EntrevistaTable.tsx`):** Renderizado de las columnas inspiradas en la ficha `P-EVA-02` (`InformeScreeningCard.tsx` / `informe_entrevista_ia`): *Candidato*, *Experiencia Consolidada*, *Alineación y Motivadores*, *Pretensión y Condiciones*, *Auditoría de Veracidad* (micro-chips de Inconsistencias y Fortalezas validadas), *NOTAS RECLUTADOR EVALUACIONES* y *Acciones* (*Detalles*, *Analizar / Re-analizar Transcripción*).
    *   **Densidad de Vista (`DensitySelector`) y Desplegables Hover `?` (Glassmorphism):** Habilitación del selector de densidad (`Compacta` vs `Expandida`) e insignias `HelpCircle` (`?`) con popovers flotantes con backdrop-blur al pasar el cursor sobre las celdas de texto extenso o estructurado.
    *   **Ordenamiento por Columnas de Entrevista:** Habilitación de `handleSort` para ordenar dinámicamente por `exp_entrevista`, `alig_entrevista`, `pretension` y `auditoria`.
    *   **Modal Human-in-the-Loop Integrado (`AnalizarTranscripcionModal`):** Disparo directo del modal de análisis de transcripción de screening (`ID: M-TRN-01`) desde los botones de acción de la tabla en `P-EVA-01` con refresco automático del pipeline.
    *   **Pruebas Automatizadas:** Creación de la suite `tests/lista_entrevista.test.js` (16/16 tests pasando al 100% en la suite combinada de evaluación y test de personalidad).

*   **23/08/2026:** Mejoras en Vistas "Lista Test Personalidad" y "Lista Detallada" (ID: `P-EVA-01`) (`TestPersonalidadTable.tsx` y `page.tsx`):
    *   **Ordenamiento por Cabeceras (Ascendente / Descendente):** Implementada la alternancia interactiva al hacer clic sobre los títulos de las columnas (*Candidato*, *Arquetipo Extraído*, *Dimensiones Psicométricas*, *Veredicto de Alineación (Cultural Fit)* y *NOTAS RECLUTADOR EVALUACIONES*), con soporte visual de flechas de ordenamiento y reutilización del estado `sortField`/`sortDirection` en `src/app/evaluacion/page.tsx`.
    *   **Reemplazo y Renombrado de Columna:** Sustituida la columna "FECHA ANÁLISIS" por **"NOTAS RECLUTADOR EVALUACIONES"** en la vista Lista Test Personalidad, y renombrada la cabecera "NOTAS RECLUTADOR" a **"NOTAS RECLUTADOR EVALUACIONES"** en la vista "Lista Detallada", habilitando además su ordenamiento interactivo con `toggleSort("notes")`.
    *   **Desplegables e Insignias `?` al pasar el cursor (Hover Tooltips Glassmorphism):** Adición de la insignia con el símbolo `?` (`HelpCircle` de `lucide-react`) y popovers flotantes oscuros translúcidos al posicionar el ratón sobre las celdas de *Veredicto de Alineación (Cultural Fit)* y *NOTAS RECLUTADOR EVALUACIONES* (tanto en Lista Test Personalidad como en Lista Detallada), con posicionado inteligente (`isTopRow`) para evitar cortes superiores.
    *   **Pruebas Automatizadas:** Actualización de las suites `tests/test_personalidad.test.js` y `tests/evaluacion.test.js` (12/12 pruebas unitarias pasando al 100%) validando el ordenamiento, el renombrado de cabecera y la presencia de los desplegables hover `?`.
...
*   **21/08/2026:** Implementación del Módulo Test de Personalidad / Cognitive Fit Vision (CFV) - V3 ("Análisis Psicométrico con IA"):
    *   **Inferencia Multimodal & Server Actions (`src/actions/pipeline.ts`):** Creación de `analizarTestPersonalidadAction` (integrada con el backend REST `POST /api/v1/pipeline/:id/analizar-personalidad` enviando la captura de imagen en RAM `.png`, `.jpg`, `.jpeg`, `.webp` <5MB) y `actualizarTestPersonalidadAction` (`PATCH /api/v1/pipeline/:id`) para edición manual Human-in-the-Loop.
    *   **Modal Animado de Ingesta ("Efecto Wow") (`AnalizarTestPersonalidadModal.tsx`):** Componente modal con diseño glassmorphism, arrastre/soltado de archivos, animación fluida en 3 pasos secuenciales (*1. Lectura y validación*, *2. Inferencia Gemini IA*, *3. Sincronización*) e interceptación estricta de errores HTTP (400 formato, 429 cuota/Resource Exhausted, 500 y cortes de red) desplegando notificaciones Toasts/Banners sin generar datos mock falsos.
    *   **Nueva Vista "Lista Test Personalidad" (ID: `P-EVA-01`) (`TestPersonalidadTable.tsx`):** Adición del 3er botón selector en la barra superior de `/evaluacion` habilitando el selector de densidad (`DensitySelector`: `Compacta` vs `Expandida`). En la vista tradicional "Lista Detallada", se incorporó una columna dedicada "Test Personalidad" con el arquetipo micro-chip (ej. `ENTJ-A`) y 1 línea de encaje cultural.
    *   **Smart Scorecard & Barras Bivalentes (`TestPersonalidadCard.tsx`):** Tarjeta homologada en la ficha del candidato (`P-EVA-02`) con arquetipo, análisis de encaje cultural, timestamp y las 5 dimensiones psicométricas en barras progresivas bivalentes (*Extravertido ↔ Introvertido*, *Intuitivo ↔ Observador*, *Racional ↔ Emocional*, *Planificador ↔ Prospectivo*, *Asertivo ↔ Turbulento*) con formulario de edición manual integrado.
    *   **Pruebas Automatizadas:** Creación de la suite `tests/test_personalidad.test.js` (11/11 tests de evaluacion/test_personalidad pasando al 100%) y verificación de `npx tsc --noEmit` sin errores de compilación.
...
*   **17/08/2026:** Implementación del Selector de Densidad de Vista (Vista Compacta Híbrida vs. Vista Expandida) y Persistencia Extendida en Pantalla P-DIS-01 ("Lista Screening IA"):
    ...
    *   **Fix de Tipado para Build de Producción (Firebase App Hosting):** Homologación del tipo del parámetro `state` y `extra` en la interfaz `ScreeningIATableProps` (`ScreeningCandidate['phase1State']` y `Partial<ScreeningCandidate>`), solucionando el error de compilación de TypeScript durante el `next build` en el despliegue a Firebase App Hosting.
...
*   **27/08/2026:** Ficha Técnica de Presentación a Cliente a PDF (Dossier PDF V2):
    *   **Generación de Dossier PDF Interactivo en Frontend:** Implementación del botón **"Generar Ficha (PDF)"** en la barra de navegación superior de las páginas detalladas del candidato (`/evaluacion/[id]`, `/presentacion/[id]`, `/cierre/[id]`, `/talento/[id]`).
    *   **Modal de Parámetros e Identidad Institucional (P-CFG-01):** Creación del componente `GenerarFichaPdfModal.tsx` que permite seleccionar dinámicamente los bloques a incluir (Resumen IA, Test de Personalidad CFV, Pretensiones Salariales, Assessment Manual, Bitácora Reclutador, Trayectoria Laboral) y ofrece el interruptor de *Modo Ciego* (`anonimizar_candidato`). El documento PDF consolida automáticamente el Branding de la Agencia (Logo, Nombre comercial y sello *"Powered by Azul ATS"*) desde la configuración maestra global `configuracion_agencia/p-cfg-01`.
    *   **Animación de Progreso ("Efecto Wow") y Descarga Transparente:** Interfaz interactiva de 4 estados secuenciales (*Recopilando datos* ➔ *Redactando con IA* ➔ *Ensamblando PDF con Branding* ➔ *Descarga completada*). Invoca la Server Action `generarFichaPdfAction` enviando `POST /api/v1/pipeline/:id/generar-ficha-pdf`, convierte el binario `application/pdf` a `Blob` y fuerza la descarga automática en el navegador con el patrón estandarizado `Ficha_Tecnica_[NombreCandidato]_[Puesto].pdf` sin recarga de página.
    *   **Memoria de Preferencias:** Persistencia automática de las opciones seleccionadas por el usuario en `localStorage` (`ats_ficha_pdf_preferences`).
    *   **Pruebas Automatizadas:** Creación de la suite de pruebas unitarias `tests/generar_ficha_pdf.test.js` (7/7 pruebas en verde) certificando el flujo de Server Actions, modal UI, patrones de descarga y compatibilidad con el modo mock.
...
*   **20/08/2026:** Corrección de Build TypeScript: Propiedad anidada en Búsqueda (`ID: P-DIS-02`) (`/descubrimiento/[id]`):
    *   **Acceso a Propiedad Anidada Jerárquica (`estado_sla.estado_busqueda`):** Reemplazado el acceso directo `activeBusquedaObj?.estado` (inexistente en el esquema de `Busqueda`) por la ruta anidada `activeBusquedaObj?.estado_sla?.estado_busqueda` en `src/app/descubrimiento/[id]/page.tsx` (tanto en la evaluación condicional como en el renderizado dentro del `<span>`).
    *   **Actualización de Interfaz e Inferencia:** Extendida la interfaz `Busqueda` en `src/actions/busquedas.ts` para incluir las definiciones opcionales `estado_sla` y `ubicacion`, actualizando el mapeo de `getBusquedasAPI()` para garantizar el retorno coherente de estos campos.
    *   **Pruebas Automatizadas:** Creación de la suite de pruebas `tests/busqueda_p_dis_02.test.js` (2/2 tests pasando) verificando el acceso anidado seguro y la asignación por defecto de `estado_sla`.
